...
# ── GA-calibrated weights (Notebook 8, corr-seed arm) ────────────────────────
GA_WEIGHTS = {
    "Global (GA-calibrated)": {
        "CATE":0.3484,"SPILLOVER":0.0058,"STRATEGIC":0.0347,"WINDOW":0.4128,"HIST_RATE":0.1983,
    },
    "Carrier: Mainline": {
        "CATE":0.3441,"SPILLOVER":0.0520,"STRATEGIC":0.0219,"WINDOW":0.4464,"HIST_RATE":0.1355,
    },
    "Carrier: LCC": {
        "CATE":0.0959,"SPILLOVER":0.3305,"STRATEGIC":0.0055,"WINDOW":0.5682,"HIST_RATE":0.0000,
    },
    "Carrier: ULCC": {
        "CATE":0.3506,"SPILLOVER":0.0349,"STRATEGIC":0.0216,"WINDOW":0.5899,"HIST_RATE":0.0030,
    },
    "Carrier: Regional": {
        "CATE":0.0000,"SPILLOVER":0.0099,"STRATEGIC":0.0000,"WINDOW":0.9308,"HIST_RATE":0.0593,
    },
    "Hub status: Hub-to-Hub": {
        "CATE":0.4067,"SPILLOVER":0.0008,"STRATEGIC":0.0034,"WINDOW":0.4410,"HIST_RATE":0.1481,
    },
    "Hub status: Hub-to-Spoke": {
        "CATE":0.0702,"SPILLOVER":0.3161,"STRATEGIC":0.1224,"WINDOW":0.3985,"HIST_RATE":0.0929,
    },
    "Hub status: Spoke-to-Spoke": {
        "CATE":0.4117,"SPILLOVER":0.0007,"STRATEGIC":0.0001,"WINDOW":0.5871,"HIST_RATE":0.0003,
    },
    "Time: Afternoon Peak (15-19)": {
        "CATE":0.1336,"SPILLOVER":0.1060,"STRATEGIC":0.0152,"WINDOW":0.6647,"HIST_RATE":0.0804,
    },
    "Time: Evening/Night (19+)": {
        "CATE":0.0720,"SPILLOVER":0.4398,"STRATEGIC":0.0442,"WINDOW":0.3084,"HIST_RATE":0.1356,
    },
    "Time: Midday (11-15)": {
        "CATE":0.2931,"SPILLOVER":0.0244,"STRATEGIC":0.0072,"WINDOW":0.1772,"HIST_RATE":0.4981,
    },
    "Time: Morning (5-11)": {
        "CATE":0.3411,"SPILLOVER":0.0001,"STRATEGIC":0.0000,"WINDOW":0.0009,"HIST_RATE":0.6580,
    },
}

CARRIER_DATA = pd.DataFrame([
    {"Carrier":"AA","Type":"Mainline","CATE":32.72,"CI_LB":24.1,"CI_UB":41.3},
    {"Carrier":"DL","Type":"Mainline","CATE":34.32,"CI_LB":25.8,"CI_UB":42.8},
    {"Carrier":"UA","Type":"Mainline","CATE":31.84,"CI_LB":23.2,"CI_UB":40.4},
    {"Carrier":"AS","Type":"Mainline","CATE":32.90,"CI_LB":24.3,"CI_UB":41.5},
    {"Carrier":"HA","Type":"Mainline","CATE":39.64,"CI_LB":31.0,"CI_UB":48.3},
    {"Carrier":"WN","Type":"LCC",     "CATE":34.88,"CI_LB":26.4,"CI_UB":43.4},
    {"Carrier":"B6","Type":"LCC",     "CATE":38.39,"CI_LB":29.8,"CI_UB":47.0},
    {"Carrier":"NK","Type":"ULCC",    "CATE":39.73,"CI_LB":31.1,"CI_UB":48.4},
    {"Carrier":"F9","Type":"ULCC",    "CATE":39.52,"CI_LB":30.9,"CI_UB":48.1},
    {"Carrier":"G4","Type":"ULCC",    "CATE":42.66,"CI_LB":34.0,"CI_UB":51.3},
    {"Carrier":"OO","Type":"Regional","CATE":43.91,"CI_LB":35.3,"CI_UB":52.5},
    {"Carrier":"YX","Type":"Regional","CATE":40.96,"CI_LB":32.4,"CI_UB":49.5},
    {"Carrier":"9E","Type":"Regional","CATE":44.02,"CI_LB":35.4,"CI_UB":52.6},
    {"Carrier":"MQ","Type":"Regional","CATE":43.14,"CI_LB":34.5,"CI_UB":51.8},
    {"Carrier":"OH","Type":"Regional","CATE":43.25,"CI_LB":34.6,"CI_UB":51.9},
    {"Carrier":"YV","Type":"Regional","CATE":41.32,"CI_LB":32.7,"CI_UB":49.9},
])
...
# ── Individual benchmark curves K=1-20 (all real computed values)
# Source: notebook8_results/global_ga_k_full_1_20.csv
BENCHMARK_METHODS_ALL = {
    ...
    "Longest delay first":  [ 3.46, 7.16,10.77,13.92,16.72,19.27,21.60,23.75,25.74,27.60,
                              29.34,31.00,32.58,34.04,35.46,36.80,38.10,39.32,40.51,41.66],
    "Earliest dep first":   [ 3.33, 6.93,10.60,13.84,16.77,19.47,21.95,24.26,26.42,28.44,
                              30.34,32.14,33.84,35.45,36.99,38.48,39.89,41.25,42.56,43.81],
    "Highest spillover":    [ 3.44, 7.06,10.73,13.94,16.82,19.44,21.85,24.07,26.15,28.10,
                              29.93,31.67,33.30,34.83,36.31,37.71,39.07,40.37,41.62,42.80],
    "CATE only":            [ 3.33, 6.61, 9.91,12.75,15.28,17.58,19.68,21.61,23.41,25.08,
                              26.68,28.18,29.60,30.94,32.23,33.47,34.65,35.80,36.90,37.96],
    "Random":               [ 3.39, 6.77,10.17,13.10,15.71,18.07,20.24,22.24,24.09,25.81,
                              27.44,28.99,30.43,31.80,33.13,34.37,35.58,36.74,37.86,38.94],
    "Hub first":            [ 3.19, 6.46, 9.79,12.70,15.31,17.67,19.84,21.83,23.68,25.43,
                              27.07,28.62,30.08,31.48,32.80,34.06,35.28,36.45,37.57,38.66],
}
...
def saw_score(features_matrix, weights_dict, eps=1e-9):
    ...
    w = np.array([
        weights_dict["CATE"], weights_dict["SPILLOVER"], weights_dict["STRATEGIC"],
        weights_dict["WINDOW"], weights_dict["HIST_RATE"],
    ])
    return X_norm @ w
...
# ── Header ────────────────────────────────────────────────────────────────────
st.title("C-FRPD — Causal Flight Recovery Prioritization Dashboard")
st.markdown(
...
flight is also disrupted. A CATE of 30.57 pp means a prior disruption raises the next
flight's disruption probability by 30.57 percentage points. Estimated using a causal forest
with airport-day confounding controls, validated by three placebo tests.

**What is the SAW priority score?**
Simple Additive Weighting (SAW) ranks disrupted flights using five criteria: CATE propagation
risk, downstream spillover, route strategic value, recovery window, and historical disruption
rate. Each criterion is min-max normalized per scenario, then weighted and summed. Weights are
calibrated by a genetic algorithm (GA). Both seeding strategies converged to near-identical
final weights (fitness gap = 0.0008), confirming a robust optimum.

**The four tabs:**
- **Global Causal Intelligence** — System-wide propagation effects and causal identification chain.
- **Airline Command View** — Carrier-specific CATE vs. system average.
- **C-FRPD Prioritization Engine** — GA-calibrated SAW ranking vs. 6 benchmarks at any K=1-20.
- **Live Flight Triage** — Enter today's disrupted flights for a real-time SAW priority ranking.
...
with tab1:
    ...
    st.subheader("CATE Heatmap — Carrier Type x Time of Day")
...
with tab2:
    ...
    with c4:
        frag = "HIGH" if diff > 3 else ("LOW" if diff < -3 else "Average")
    ...
    with col_r:
        st.subheader(f"{selected} — CATE by Route x Time")
        ch = D["carr_heat"]
...
with tab3:
    ...
    if ct_filter == "All":
        ...
        with m1:
            st.markdown(
                ...
                f'<div class="metric-value">{ga_pct:.2f}%</div>'
                f'<div class="metric-label">C-FRPD captured (K={k_val})</div>'
                f'</div>', unsafe_allow_html=True)
        with m2:
            card_cls = "low-risk" if gap >= 0 else "high-risk"
            st.markdown(
                f'<div class="metric-card {card_cls}">'
                f'<div class="metric-value">{sign}{gap:.2f} pp</div>'
                f'<div class="metric-label">vs best benchmark ({best_name})</div>'
                f'</div>', unsafe_allow_html=True)
        with m3:
            st.markdown(
                f'<div class="metric-card">'
                f'<div class="metric-value">{protected:,}</div>'
                f'<div class="metric-label">Downstream flights protected</div>'
                f'</div>', unsafe_allow_html=True)
        with m4:
            st.markdown(
                f'<div class="metric-card">'
                f'<div class="metric-value">{prega_pct:.2f}%</div>'
                f'<div class="metric-label">Pre-GA SAW baseline (K={k_val})</div>'
                f'</div>', unsafe_allow_html=True)

    st.subheader(
        f"Benchmark Comparison at K={k_val}" +
        ("" if ct_filter == "All" else f" -- {ct_filter}")
    )

    # ── Bar chart ─────────────────────────────────────────────────────────────
    if ct_filter == "All":
        bar_data = {m: vals[k_val-1] for m, vals in BENCHMARK_METHODS_ALL.items()}
        bar_df = (pd.DataFrame({"method": list(bar_data.keys()),
                                "pct":    list(bar_data.values())})
        ...
        if len(bench_k) > 0:
            bench_k = bench_k.copy()
            bench_k["method"] = bench_k["method"].replace(
                {"TOPSIS (DSS)": "C-FRPD (GA-calibrated)"})
            fig10 = go.Figure(go.Bar(
                x=bench_k["method"].tolist(),
                y=bench_k["pct_captured"].tolist(),
                marker_color=[METHOD_COLORS.get(m,"#7f7f7f") for m in bench_k["method"]],
                text=[f"{v:.2f}%" for v in bench_k["pct_captured"]], textposition="outside"
            ))
            fig10.update_layout(height=380,
                                yaxis_title="Downstream disruptions captured (%)",
                                margin=dict(l=0,r=20,t=20,b=120),
            ...
            st.info(
                f"Carrier-type breakdown available for K=1-10 only. "
                f"Switch to 'All' for K=1-20."
            )
    ...
    fig11 = go.Figure()
    for method, vals in BENCHMARK_METHODS_ALL.items():
    ...
    fig11.update_layout(
        height=400,
        xaxis_title="Intervention budget K",
        yaxis_title="Downstream disruptions captured (%)",
        margin=dict(l=0,r=20,t=20,b=40),
        plot_bgcolor="white",
    ...
    with col_w1:
        ...
        st.caption(
            "GA reallocates weight from spillover (0.319 to 0.006) to CATE (0.061 to 0.348). "
    ...
    with col_w2:
        ...
        policy_summary = pd.DataFrame([
            {"Policy": "Global (GA-calibrated)",  "Captured %": 11.06, "vs Best benchmark": "+0.29 pp"},
            ...
            {"Policy": "Airport: Large (dagger)",  "Captured %":  4.13, "vs Best benchmark": "+0.20 pp"},
        ])
        st.dataframe(policy_summary, use_container_width=True, hide_index=True, height=380)
        st.caption("Marginally validated (+0.20 pp). Airport Medium/NonHub/Small inherit Global policy.")
    ...
    if missing:
        ...
        queue["SAW_SCORE"] = saw_score(
            X_q, GA_WEIGHTS["Global (GA-calibrated)"]).round(4)
...
2. **Set K** -- your actual available recovery capacity.
3. **Add each disrupted flight** using the form below.
4. **Review the SAW priority ranking** -- the top-K flights are your recommended interventions.

**Field guide:**
- Route type: Hub-to-Hub, Hub-to-Spoke, or Spoke-to-Spoke
...
**Tip:** Add at least 3-5 flights to make the ranking meaningful.
    ...
    st.info(
        "CATE auto-estimated from carrier x route x time heatmap (2015-2019 training data). "
    ...
    with st.form("flight_input_form"):
        ...
        with col1:
            f_carrier   = st.selectbox(
                "Carrier", sorted(CARRIER_DATA["Carrier"].tolist()),
        ...
        with col2:
            f_route     = st.selectbox("Route type",
                                       ["Hub-to-Hub","Hub-to-Spoke","Spoke-to-Spoke"])
            f_tier      = st.selectbox("Origin tier",
        ...
        if submitted:
            if len(st.session_state.flights) >= 10:
                ...
                st.success(
                    f"Flight {f_carrier} {f_origin.upper()} to {f_dest.upper()} added.")
    ...
    if len(st.session_state.flights) > 0:
        ...
        st.subheader(
            f"Step 3 -- C-FRPD SAW Priority Ranking (Policy: {policy_choice})")
        if len(st.session_state.flights) < 2:
            ...
            st.dataframe(
                results[["Priority","Carrier","Origin","Dest","Route type",
                          "CATE (pp)","Spillover","Rec Window",
                          "SAW Score"]].reset_index(drop=True),
            ...
            st.markdown("---")
            st.subheader(
                f"Top Priority: {top['Carrier']} {top['Origin']} to {top['Dest']}")
            c1, c2, c3, c4 = st.columns(4)
            with c1:
                st.markdown(
                    f'<div class="metric-card high-risk">'
                    f'<div class="metric-value">{top["CATE (pp)"]:.1f} pp</div>'
                    f'<div class="metric-label">Causal propagation risk</div>'
                    f'</div>', unsafe_allow_html=True)
            with c2:
                st.markdown(
                    f'<div class="metric-card">'
                    f'<div class="metric-value">{int(top["Spillover"])}</div>'
                    f'<div class="metric-label">Downstream flights at risk</div>'
                    f'</div>', unsafe_allow_html=True)
            with c3:
                st.markdown(
                    f'<div class="metric-card">'
                    f'<div class="metric-value">{int(top["Rec Window"])} min</div>'
                    f'<div class="metric-label">Recovery window remaining</div>'
                    f'</div>', unsafe_allow_html=True)
            with c4:
                st.markdown(
                    f'<div class="metric-card low-risk">'
                    f'<div class="metric-value">{top["SAW Score"]:.3f}</div>'
                    f'<div class="metric-label">SAW priority score</div>'
                    f'</div>', unsafe_allow_html=True)
            ...
            fig_radar.add_trace(go.Scatterpolar(
                ...
                name=f"{top['Carrier']} {top['Origin']} to {top['Dest']}"
            ))
            fig_radar.add_trace(go.Scatterpolar(
            ...
            with col_note:
                ...
                if top_vals[0] > 60:
                    reasons.append(
                        f"High causal propagation risk ({top['CATE (pp)']:.1f} pp)")
                if top_vals[1] > 60:
                    reasons.append(
                        f"Large downstream spillover ({int(top['Spillover'])} flights)")
                if top_vals[3] > 60:
                    reasons.append(
                        f"Generous recovery window ({int(top['Rec Window'])} min)")
                if top_vals[4] > 60:
                    reasons.append(
                        f"Chronically disrupted route "
                        f"({top['Hist Rate']*100:.1f}% 30-day rate)")
                if not reasons:
                    reasons.append(
                        "Highest composite SAW score across all criteria")
                for r in reasons:
                    st.markdown(f"- {r}")
                st.markdown("---")
                st.markdown("**Active policy weights:**")
                w = active_weights
                st.markdown(
                    f"- CATE: **{w['CATE']:.3f}**  \n"
            ...
            st.caption(
                f"SAW scoring with '{policy_choice}' weights (GA-calibrated, "
                f"holdout-validated). CATE estimated from carrier x route x time "
                f"heatmap (2015-2019 training data). "
